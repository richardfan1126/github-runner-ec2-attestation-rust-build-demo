...
from scripts.parse_markers import parse_artifact_name_marker, parse_sha256_marker
...
# We filter out lines starting with "BINARY_SHA256:" or "BINARY_ARTIFACT_NAME:"
...
_safe_line_char = st.text(
    ...
    and not s.startswith("BINARY_ARTIFACT_NAME:")
)

surrounding_lines = st.lists(_safe_line_char, min_size=0, max_size=5)

# Valid artifact name: non-empty, no newlines, and after stripping still
# non-empty.  Also must not look like a SHA256 marker value (64 hex chars)
# to keep parsing unambiguous.
artifact_name = st.text(
    alphabet=st.characters(
        whitelist_categories=("L", "N", "P", "S", "Z"),
        blacklist_characters="\n\r",
    ),
...
@settings(max_examples=100)
@given(name=artifact_name, before=surrounding_lines, after=surrounding_lines)
def test_artifact_name_marker_roundtrip(
    name: str,
    before: list[str],
    after: list[str],
) -> None:
    """Embedding a BINARY_ARTIFACT_NAME marker in arbitrary stdout and
    parsing it back SHALL return the original value (stripped).

    **Validates: Requirements 2.5, 4.2, 4.3, 4.4**
    """
    marker_line = f"BINARY_ARTIFACT_NAME:{name}"
    stdout = "\n".join([*before, marker_line, *after])

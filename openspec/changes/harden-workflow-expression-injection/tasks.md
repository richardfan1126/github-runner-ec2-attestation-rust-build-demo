...
- [x] 1.1 Capture a `BINARY_OCI_REF` value from a prior successful run (or the remote
...
- [x] 1.2 Inventory every `${{ … }}` interpolation inside a `run:` body in
...
- [x] 2.1 Add the "Untrusted Inputs Are Not Interpolated Into Run Bodies" requirement and
...
- [x] 2.2 Validate the delta: `openspec validate harden-workflow-expression-injection --strict`.

## 3. Shell injection — env: indirection (D1)

- [x] 3.1 Step 7 (Run Remote Executor Caller): move `inputs.server_url`,
...
- [x] 3.2 Step 13 (Push OCI artifact): move `inputs.commit_hash`,
  `github.repository_owner`, and `github.event.repository.name` into `env:`; reference
  `"$VAR"`.
- [x] 3.3 Step 16 (Print job summary): move `steps.parse_markers.outputs.binary_sha256`
  and the `steps.oras_push.outputs.*` / `steps.attest.outcome` values into `env:`;
  reference `"$VAR"`.
- [x] 3.4 Confirm no `run:` body in Steps 7/13/16 contains a bare `${{ … }}` expression.

## 4. Python injection — os.environ (D2)

- [x] 4.1 Step 11 (Create provenance manifest): move `binary_sha256`, the derived
  `REPO_URL`/`COMMIT_HASH`, and `github.run_id` into `env:`.
- [x] 4.2 Rewrite the inline `python3 -c` script to read `os.environ['BINARY_SHA256']`,
...
- [x] 5.1 Step 8 (Parse stdout markers): after extracting `BINARY_OCI_REF`, validate it
...
- [x] 5.2 Step 9 (Pull temporary binary) and Step 17 (Get temporary package version ID):
...
- [x] 6.1 Grep `attested-rust-build.yml` for `${{ inputs.` and
...
- [x] 6.2 Run `actionlint` (if available) on the workflow; resolve any new findings.
- [x] 6.3 Re-run `openspec validate harden-workflow-expression-injection --strict`.
- [x] 6.4 Update `SECURITY_REVIEW.md` finding 1 and the related "Coverage gaps" entries

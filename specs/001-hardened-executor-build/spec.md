...
- Q: Is `RUSTUP_HOME` a writable scratch target or a read-only image path the build never writes? → A: Read-only image path — only `CARGO_HOME` and `CARGO_TARGET_DIR` (plus source copy, downloads, oras auth) are written under scratch. The build invokes the real `cargo`/`rustc` binaries directly (not the rustup proxy shims), so `RUSTUP_HOME` is never written and stays at its read-only image path.
- Q: How is an out-of-scratch write surfaced — does the build script actively classify it as "outside scratch," or is it OS-enforced? → A: OS-enforced. The read-only rootfs/workspace denies the stray write (`EROFS`); the build fails non-zero and surfaces the failing command's error (which includes the offending path). The script does NOT itself detect or classify "outside scratch." The script's one proactive, attributable filesystem check is the up-front scratch **write-probe** (which names the scratch path if scratch is unwritable).
...
- **FR-010**: The build MUST perform all writes — including the Rust toolchain's writable cargo home (`CARGO_HOME`), the cargo build target directory (`CARGO_TARGET_DIR`), the staged source copy, downloaded artifacts, and credential/scratch files — under the executor-provided writable scratch mount only. `RUSTUP_HOME` is **NOT** a write target: the build invokes the real `cargo`/`rustc` binaries directly (not the rustup proxy shims), so `RUSTUP_HOME` remains at its read-only image path and is never written at run time. This enumeration is intended to be **exhaustive** of the build's write targets: together with FR-008 (read-only rootfs) and FR-009 (read-only workspace), every write the build performs lands in scratch and nowhere else.
...
- **FR-016**: If a required tool is missing or a write is attempted outside the scratch mount, the build MUST fail with a clear, attributable error rather than silently continuing or attempting a privileged fallback. A "clear, attributable error" MUST: (a) cause a non-zero exit so the workflow fails, and (b) emit a message naming **which** condition occurred and the offending detail. The attribution mechanism differs by case:
  - **Missing tool** — the build script preflights each required tool and, on absence, fails non-zero naming the specific tool (e.g., `oras`). *(script-enforced)*
  - **Write outside scratch** — enforcement is delegated to the executor's **read-only** rootfs/workspace: the kernel denies the stray write (`EROFS` / "Read-only file system") and the build fails non-zero, surfacing the failing command's error (which includes the offending path). The script does **not** itself classify a write as "outside scratch" or pre-validate write targets; it relies on read-only enforcement and never retries with privilege. *(OS-enforced)* As the one proactive, script-level filesystem check, the script write-probes the **scratch** root up front and, if scratch is not writable, fails non-zero naming the scratch path.
  - **Scratch exhaustion** — on a compile/link write failing for lack of space, the build fails non-zero naming that the writable scratch mount was filled. *(script-enforced over the OS error)*
...
- **Writable scratch mount**: The executor-provided tmpfs/scratch directory — the only writable location. Key attributes: hosts the cargo home (`CARGO_HOME`), the cargo target directory, the staged source copy, downloads, and credentials; must meet the documented minimum size. (`RUSTUP_HOME` is not hosted here — it stays at its read-only image path; see FR-010.)

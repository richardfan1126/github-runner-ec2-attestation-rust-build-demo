...
- A `GITHUB_TOKEN` (PAT or Actions token) with `packages:write` for the push scenario (Scenario C). Scenarios A–B need no network.
...
docker run --rm --network none hardened-build:local sh -lc '
  set -e
  id
...
```bash
# Missing tool: shadow oras with an empty PATH entry → names the tool
docker run --rm --user 65534:65534 --read-only --tmpfs /tmp:rw,size=4g \
  -v "$PWD":/workspace:ro -e PATH=/nonexistent \
  -e GITHUB_TOKEN=x -e GITHUB_REPOSITORY=o/r -e COMMIT_SHA=deadbeef \
  hardened-build:local bash /workspace/scripts/build-rust.sh; echo "exit=$?"

# Scratch too small: shrink tmpfs below the build's need → names scratch exhaustion
docker run --rm --user 65534:65534 --read-only --tmpfs /tmp:rw,size=8m \
  -v "$PWD":/workspace:ro \
  -e GITHUB_TOKEN=x -e GITHUB_REPOSITORY=o/r -e COMMIT_SHA=deadbeef \
  hardened-build:local bash /workspace/scripts/build-rust.sh; echo "exit=$?"
```

**Expected**: non-zero exit in each case; stderr names the specific condition (missing tool name / scratch exhausted) and offending detail; **no** `BINARY_SHA256`/`BINARY_OCI_REF` emitted. (Validates FR-016, FR-016a.)
...
docker run --rm --user 65534:65534 --read-only --tmpfs /tmp:rw,size=4g \
  ...
  -e COMMIT_SHA="$(git rev-parse HEAD)" \
  hardened-build:local bash -lc '
    /workspace/scripts/build-rust.sh & pid=$!
...
```bash
pip install -e ".[dev]"
pytest tests/test_build_image_hardening.py -q
```
...
- [ ] Scenario A: tools present + pinned + runnable as `65534`, offline.
- [ ] Scenario B: attested artifact pushed under full hardened constraints; markers correct.
- [ ] Scenario C: each failure exits non-zero with an attributable message and no markers.
- [ ] Scenario D: peak scratch measured and recorded.
- [ ] Static tests pass.
- [ ] README lets a fresh operator configure image ref + scratch with no maintainer help.

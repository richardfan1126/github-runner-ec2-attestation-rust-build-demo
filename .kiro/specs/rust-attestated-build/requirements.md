...
- **Attestation_Document**: A COSE Sign1 document produced by the AWS Nitro Enclave that cryptographically proves the execution environment identity and integrity. For execution-acceptance attestations, the `user_data` field contains a JSON object with `repository_url`, `commit_hash`, `script_path`, `script_env_hash`, `execution_id`, and `timestamp`. For output-integrity attestations, the `user_data` field contains a JSON object with `output_digest` (SHA-256 hex digest of the canonical output) and `execution_id`.
- **Encrypted_Error_Envelope**: An encrypted JSON response returned by the Remote_Executor after successful request decryption when an application-level error occurs. Contains an `error` description and `error_code` field (the HTTP status code that would have been returned). Returned with HTTP 200 at the transport layer so observers cannot distinguish errors from successes. Pre-decryption errors (malformed JSON, decryption failure) remain as plaintext HTTP errors.
...
2. THE Caller module SHALL include the collected `script_env` dictionary in the encrypted /execute payload alongside the existing fields (repository_url, commit_hash, script_path, github_token, oidc_token, nonce). THE `nonce` field is mandatory — the Remote_Executor rejects requests with missing or empty nonces with HTTP 400.
